...
class SaveScreen extends Component {
    ...
    saveScreening() {
        const querydata = this.props.querydata;
        let myScreeningName = 'myScreeningSearch';

        let myScreening = localStorage.getItem(myScreeningName) || '';
        ...
        localStorage.setItem(myScreeningName,myScreening);
    ...
    render() {
        ...
        return (
            <div className="xh-fl xh-screen-left">
                ...
                {/*<!--保存按钮-->*/}
                {querydata && querydata.from == "screening" && !this.props.dontsave?<a href="javascript:;" className="xh-fl xh-btn xh-preservation-btn" onClick={()=>{this.saveScreening()}}>保存筛选</a>:""}
            </div>
        )
    }
}
