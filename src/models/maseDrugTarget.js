...
export default {
    ...
    subscriptions: {
        setup({dispatch, history}) {
            history.listen(location => {
                ...
                if (location.pathname === '/medicalretrieval/maseDrugTarget') {
                    dispatch({
                        type:'renderList',
                        payload:{
                            offset:0
    ...
    effects: {
        ...
        *renderList({payload},{call,put,select}) {
            ...
            //获取缓存信息
            const listCache = yield select(({ maseDrugTarget }) => maseDrugTarget.listCache);
            ...
            const { data } = yield call( axios , 'http://api2.drugsea.cn/jiyao/list' , options );
