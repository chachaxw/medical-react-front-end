...
export default {
    ...
    subscriptions: {
        setup({dispatch, history}) {
            history.listen(location => {
                ...
                if (location.pathname === '/medicalretrieval/instructions') {
                    dispatch({
                        type:'renderList',
                        payload:{
                            offset:0
    ...
    effects: {
        ...
        *renderList({payload},{call,put,select}) {
            ...
            //获取缓存信息
            const listCache = yield select(({ instructions }) => instructions.listCache);
            ...
            const { data } = yield call( axios , 'http://api3.drugsea.cn/global_drugs/list' , options );
