...
export default {
    ...
    subscriptions: {
        setup({dispatch, history}) {
            history.listen(location => {
                ...
                if (location.pathname === '/medicalretrieval/medicalInsuranceDirectory') {
                    dispatch({
                        type:'renderList',
                        payload:{
                            offset:0
    ...
    effects: {
        ...
        *renderList({payload},{call,put,select}) {
            ...
            //获取缓存信息
            const listCache = yield select(({ medicalInsuranceDirectory }) => medicalInsuranceDirectory.listCache);
            ...
            const { data } = yield call( axios , 'http://api2.drugsea.cn/yibao/list' , options );
