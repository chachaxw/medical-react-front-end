...
export default {
    ...
    subscriptions: {
        setup({dispatch, history}) {
            history.listen(location => {
                ...
                if (location.pathname === '/medicalretrieval/drugbid') {
                    dispatch({
                        type:'renderList',
                        payload:{
                            offset:0
    ...
    effects: {
        ...
        *renderList({payload},{call,put,select}) {
            ...
            //获取缓存信息
            const listCache = yield select(({ drugbid }) => drugbid.listCache);
            ...
            const { data } = yield call( axios , 'http://api2.drugsea.cn/zhaobiao/list' , options );
